...
// Electron/Web 환경에 따라 MQTT publish 분기
function publishMqttMessage(topic: string, message: string): void {
  if (isElectron()) {
    const api = getElectronAPI()
    api?.publishMQTT?.(topic, message).catch((err: Error) => {
      console.error('[OrderService] Failed to publish via IPC:', err)
    })
  } else {
...
export const orderService = {
  ...
  async createOrder(order: {
    ...
    try {
      ...
      if (order.reserveAt) {
        ...
        publishMqttMessage('reserve.request', messageArr.join('|'))
      } else if (order.drvNo) {
        ...
        publishMqttMessage('accept.order.request', messageArr.join('|'))
      } else {
        ...
        publishMqttMessage('add.order.request', messageArr.join('|'))
      }
  ...
  async updateOrder(orderId: number, order: Partial<Order>): Promise<Order> {
    try {
      ...
      if (order.drvNo) {
        ...
        publishMqttMessage('modify.order.request', messageArr.join('|'))
      }
  ...
  async cancelOrder(orderId: number, reason?: string): Promise<void> {
    try {
      ...
      publishMqttMessage('cancel.order.request', messageArr.join('|'))
